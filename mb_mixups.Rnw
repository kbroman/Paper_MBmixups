...
animals, including 297 with microbiome data, we identified three sets
...
microbiome data consisted of shotgun sequencing reads from DNA
extracted from mouse feces, in order to characterize the population of
bacterial species in the mouse gut. These sequence reads included
...
composition could similarly be affected by sample mix-ups. And if the
...
metagenomic data, there may be sufficient sequence reads from the mouse host to
identify the individual.

In a Diversity Outbred (DO) mouse project with genotype data on 500
animals \citep{keller2018}, including 297 with metagenomic sequencing
data from their fecal microbiome, we identified three sets of sample mix-ups
(two pairs and one trio) as well as at least 15 microbiome samples that appeared
to be mixtures of pairs of mice. DO mice \citep{churchill2012, svenson2012}
...
The microbiome data consisted of shotgun sequencing reads from
DNA extracted from mouse feces, in order to characterize the
population of bacterial species in the mouse gut. These sequence reads
included sufficient reads derived from the host mouse to identify the
individual. A number of microbiome samples appeared to contain a
mixture of DNA from two mice. We describe a method for identifying
sample mix-ups in such microbiome data, as well as a method for
evaluating sample mixtures in this context.
...
scripts. Reads were mapped to the mouse genome using bowtie2
...
We used a hidden Markov model (HMM) to calculate 36-state diplotype
probabilities, given the multipoint SNP genotype data, along each
chromosome. (We use the term diplotype to refer to a pair of founder
haplotypes). We then used a database of genotypes of the eight founder
...
allele at the locations of the 33 millions SNPs among the eight founder
...
potential mixtures, we considered a plot of the LRT versus the estimated mixture
...
Raw microbiome sequence reads are available at
\textbf{\emph{(some general sequence archive?)}}.
The GigaMUGA SNP genotypes are available at \textbf{\emph{(figshare?)}}.
Founder SNP genotypes are available as a
SQLite database at figshare,
\href{https://doi.org/10.6084/m9.figshare.5280229.v2}{doi:10.6084/m9.figshare.5280229.v2}.

Analysis scripts and processed data will be made available at GitHub.

An R package, R/mbmixture, for performing the mixture analysis is
available at Github: \url{https://github.com/kbroman/mbmixture}.
...
variation in the proportion of reads that were derived from the mouse
...
proportions of reads coming from the mouse host than the other two
batches.
...
eight founder strains' genotypes at 33 million SNPs to infer the
corresponding SNP genotypes in the DO mice.
...
microbiome samples (colored purple in Figure~1) where the distance to
...
nor to any sample. For DO-357 and DO-397, the genomic DNA samples were
of low quality. For DO-174 and DO-385, the microbiome data included
only a small number of reads. But another 15 microbiome samples appear
...
In contrast, in Table~S5 consider reads for microbiome sample DO-101,
split according to the SNP genotypes at genomic DNA samples DO-101 and
DO-102. Here, we see that the frequency of A reads does not depend on
...
\caption{Plot of maximum achieved likelihood ratio test statistic (for
  the test of no mixture) versus the estimated proportion from a
...
Each panel is a scatterplot of the LRT statistic versus the estimated
...
points corresponding to the 500 possible genomic DNA samples. In each
...
\caption{For selected microbiome samples, plots of the likelihood
  ratio test statistic versus the estimated proportion contaminated,
...
which should a high self-distance in Figure~1. Both of these samples
...
ribosomal RNA sequencing. It also provides the ability to identify and
...
R01DK108259 (to F.E.R.), and Postdoctoral Fellowship T15LM0075359 (to
L.L.T.).
...
# load data
pairs <- readRDS("Data/pair_results_allchr.rds")
