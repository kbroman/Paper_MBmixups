...
from the fecal microbiome of 297 samples.
...
The microbiome data consists of shotgun sequencing reads from
...
reads mapping to the mosue genome.
...
\caption{Plot of minimum distance vs.\ distance to self. Each point is a
  ...
  status: light blue points are samples that look okay, light purple points
  had few microbiome reads, salmon points had bad genomic DNA samples,
  green points appear to be mixtures, and dark purple points appear to
  be sample mix-ups.}
...
instead seek to sufficient metagenomic reads at individual SNPs in
...
approach like that developed for gene expression data. This would have
