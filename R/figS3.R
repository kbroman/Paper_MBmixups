# figure S3: details for all mixtures
...
pdf("../Figs/figS3.pdf", width=8, height=11.25, pointsize=12)
par(mfrow=c(5,3), mar=c(3.1,2.8,2.1,1.1))
ind <- paste0("DO-",  c(327, 329, 340, 344, 354,
                        346, 336, 343,  41, 358,
                        359, 362, 111, 191, 324))
for(i in seq_along(ind)) {
    if(i <= 12) {
        plot_mix(ind[i], mgp.x=c(1.4,0.3,0), mgp.y=c(1.3,0.3,0),
                 textright=(i %in% c(4:6, 8, 10:15)))
    } else {
        plot_mix(ind[i], mgp.x=c(1.4,0.3,0), mgp.y=c(1.3,0.3,0),
                 textright=(i %in% c(4:6, 8, 10:15)),
                 ylim=c(0, 1), yat=seq(0, 1, by=0.2))
    }
